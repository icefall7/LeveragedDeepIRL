...
def find_expected_svf(n_states, r, n_actions, discount,
    ...
    # print("llalalalallatraj->\n")
    # print(trajectories.get_value()[:, 0, 0])
    start_state_count = T.extra_ops.bincount(trajectories[:, 0, 0],
                                             minlength=n_states)
    # print("state count->\n")
    # print(start_state_count.eval())
...
def irl(structure, feature_matrix, n_actions, discount, transition_probability,
        trajectories, trajectories_bad, epochs, learning_rate, initialisation="normal", l1=0.1,
        l2=0.1):
    """
    ...
    trajectories: 3D array of state/action pairs. States are ints, actions
        are ints. NumPy array with shape (T, L, 2) where T is the number of
        trajectories and L is the trajectory length.
    trajectories_bad: bad demo in the same size with traj(currently)
    ...
    trajectories_bad = th.shared(trajectories_bad,borrow=True)

    # print(trajectories.get_value())
    ...
    expected_svf = find_expected_svf(n_states, r,
                                     ...
                                     trajectories)
    expected_svf_bad = find_expected_svf(n_states, r,
                                     n_actions, discount,
                                     transition_probability,
                                     trajectories_bad)
    ...
    svf_bad = maxent.find_svf(n_states, trajectories_bad.get_value())

    stat = th.shared(np.array(1, dtype=np.float64), 'stat')  # initial state = 1000
    inc = T.scalar('inc', dtype=stat.dtype)
    accumulator = th.function([inc], stat, updates=[(stat, stat + inc)])



    # Derivatives (backward propagation).
    updates = []
    temp = svf - expected_svf - stat.get_value() / 100 * (svf_bad - expected_svf_bad)
    # temp1 = th.shared(temp, borrow=True)
    α_grad = φs[-1].dot(svf - expected_svf - stat.get_value()/100*(svf_bad - expected_svf_bad)).T
    ...
    for i, W in enumerate(weights):
        w_grads, _ = th.scan(fn=grad_for_state,
                             sequences=[T.arange(n_states)],
                             non_sequences=[W, svf - expected_svf- stat.get_value()/100*(svf_bad - expected_svf_bad), r])
    ...
    for i, b in enumerate(biases):
        b_grads, _ = th.scan(fn=grad_for_state,
                             sequences=[T.arange(n_states)],
                             non_sequences=[b, svf - expected_svf- stat.get_value()/100*(svf_bad - expected_svf_bad), r])
